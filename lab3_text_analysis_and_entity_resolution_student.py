...
def simpleTokenize(string):
    ...
    """
    return [token for token in re.split(split_regex, string.lower()) if len(token) > 0]
...
def tokenize(string):
    ...
    """
    return [token for token in re.split(split_regex, string.lower()) if len(token) > 0 and not token in stopwords]
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.flatMap(lambda (a,b):b).count()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    x = vendorRDD.map(lambda (a,b):(a,b)).takeOrdered(1, key=lambda (a,b):-len(b))
    return [[x[0][0],x[0][1]]]
...
N = googleRecToToken.map(lambda (a,b):a).distinct().count()
print N
from operator import add
c1 = googleRecToToken.flatMap(lambda (a,b): list(set(b))).map(lambda x:(x,1))
tokenSumPairTuple = c1.reduceByKey(lambda a,b:a+b)
print tokenSumPairTuple.take(10)
#print idfs(googleRecToToken).count()
...
def idfs(corpus):
    ...
    N = corpus.map(lambda (a,b):a).distinct().count()
    c1 = corpus.flatMap(lambda (a,b): list(set(b))).map(lambda x:(x,1))
    tokenSumPairTuple = c1.reduceByKey(lambda a,b:a+b).map(lambda (a,b):(a, 1.0*N/b))
...
def dotprod(a, b):
    ...
    """
    return sum(dict((k, v * b[k]) for k, v in a.items() if k in b).values())
...
print similaritiesBroadcast.take(1)
...
tdList = trueDupsRDD.keys().collect()
nonDupsRDD = (sims
              .filter(lambda (a,b): a not in tdList ))
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (a,b):(a,tokenize(b))).cache()
googleFullRecToToken = google.map(lambda (a,b):(a,tokenize(b))).cache()
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (a,b):(a, tfidf(b, idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda (a,b):(a, tfidf(b, idfsFullBroadcast.value)))
...
def invert(record):
    ...
    """
    pairs = [(i, record[0]) for i in record[1] ]
    return (pairs)
amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda (a,b): invert((a,b.keys())))
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    AD = amazonWeightsBroadcast.value[amazonRec]
    #print AD
    A = {x: AD[x] for x in tokens if x in AD }
    GD = googleWeightsBroadcast.value[googleRec]
    #print "GD====="
    #print GD
    G = {x: GD[x] for x in tokens if x in GD }
    #print A
    #print G
    value = sum(dict((k, v * GD[k]) for k, v in AD.items() if k in GD).values()) / (amazonNormsBroadcast.value[amazonRec] * googleNormsBroadcast.value[googleRec] )
