...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRatings = 1.0*sum(list(IDandRatingsTuple[1])) / numRatings
    return (IDandRatingsTuple[0], (numRatings, avgRatings))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (a,b): (b[1][1],b[0],b[1][0]))
                             )                             
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .mapValues(lambda b:((b[0]-b[1]) * (b[0]-b[1])) )
                        )
...
print validationForPredictRDD.take(1)
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x:x[0] not in map(lambda x:x[1], myRatedMovies))
                      .map(lambda x:(0,x[0])))
