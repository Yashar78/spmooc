...
aSparse = SparseVector(aDense.size,range(0,aDense.size), aDense )

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(bDense.size,range(0,bDense.size), bDense )
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    indices=[]
    for rf in rawFeats:
        indices.append(OHEDict[rf])
    indices.sort()
    return(SparseVector(numOHEFeats, indices, [1.0] * len(indices) ))
...
# TODO: Replace <FILL IN> with appropriate code
#data, iterations=100, step=1.0, miniBatchFraction=1.0, initialWeights=None, regParam=0.01, 
#regType='l2', intercept=False, validateData=True
...
classOneFracTrain = OHETrainData.filter(lambda lp:lp.label==1.0).count()*1.0 / OHETrainData.count()
...
print type(model0.weights.transpose())
wt =  (model0.weights.transpose())
print wt
print type(wt)
#print (OHETrainData.take(1)[0].features).transpose()
x = OHETrainData.take(1)[0].features
#nx = np.array(x)
print wt.dot(x.toArray())
#np.dot(wt,np.array(x))
...
def getP(x, w, intercept):
    ...
    rawPrediction = w.transpose().dot(x.toArray())+intercept
...
def evaluateResults(model, data):
    ...
    preds = data.map(lambda lp:(lp.label,  getP(lp.features , model.weights, model.intercept)) )
    return(preds.map(lambda x:computeLogLoss(x[1], x[0])).mean())
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    #hashFunction(numBuckets, rawFeats, printMapping=False
    items =  point.split(',')
    features = items[1:]
    index = range(0,len(features))
    return(LabeledPoint(items[0], SparseVector(numBuckets,hashFunction(numBuckets, zip(index,features)))))
...
def computeSparsity(data, d, n):
    ...
    """
    return(data.map(lambda lp:1.0*len(lp.features.indices)/d).mean() )
...
logLossTest =evaluateResults(LogisticRegressionWithSGD.train(hashTrainData, 
                                                             iterations=500, 
                                                             step=10, 
                                                             regParam=1e-6, 
                                                             regType='l2',
...
classOneFracTrainHash = hashTrainData.filter(lambda lp:lp.label==1.0).count()*1.0 / hashTrainData.count()
logLossTestBaseline = hashTestData.map(lambda lp:computeLogLoss(classOneFracTrainHash, lp.label)).mean()
