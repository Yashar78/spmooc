...
def parsePoint(line):
    ...
    """
    items = line.split(',')
    return(LabeledPoint( float(items[0]), map(float, items[1:])))
...
def gradientSummand(weights, lp):
    ...
    """
    return np.dot( (np.dot(weights.transpose(), lp.features ) - lp.label) ,lp.features)
...
w = np.zeros(d)
gradient = parsedTrainData.map(lambda x: gradientSummand(w,x)).fold(0,add)
print gradient
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda x: gradientSummand(w,x)).fold(0,add)
...
def twoWayInteractions(lp):
    ...
    """
    return(LabeledPoint(lp.label, np.hstack((lp.features, map(lambda a:a[0]*a[1],list(itertools.product(lp.features, repeat=2)))))))
